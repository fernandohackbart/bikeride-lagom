...
class BikerEventProcessor(session: CassandraSession, readSide: CassandraReadSide)(implicit ec: ExecutionContext)
    ...
    for {
      _ <- session.executeCreateTable(
        "CREATE TABLE IF NOT EXISTS bikers ( " +
          ...
          "PRIMARY KEY (id))")
      _ <- session.executeCreateTable(
        "CREATE MATERIALIZED VIEW IF NOT EXISTS biker_email AS " +
          "SELECT " +
          "id , " +
          "name , " +
          "active "+
  ...
  private def processBikerCreated(eventElement: EventStreamElement[BikerCreated]): Future[List[BoundStatement]] = {
    insertBiker.map { ps =>
      ...
      if (!eventElement.event.biker.avatarb64.isEmpty) bindInsertBiker.setString("avatarb64", eventElement.event.biker.avatarb64.get)
      if (!eventElement.event.biker.bloodType.isEmpty) bindInsertBiker.setString("bloodtype", eventElement.event.biker.bloodType.get)
      if (!eventElement.event.biker.mobile.isEmpty) bindInsertBiker.setString("mobile", eventElement.event.biker.mobile.get)
      if (!eventElement.event.biker.email.isEmpty) bindInsertBiker.setString("email", eventElement.event.biker.email.get)
